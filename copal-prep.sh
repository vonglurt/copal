...
def log_row(when, cps, counts, seconds, averages, peaks, src):
...
def backfill(blob, args, offset=0.0, path=None):
    ...
    if not samples:
        return {"samples": 0, "added": 0, "clashed": 0, "first": None,
                "last": None}
    rows = rows_from_samples(samples, args.spans, every, args.max_gap)
    log_path = path or os.path.join(state_dir(), LOG_NAME)
    added, clashed = merge_log(log_path, log_header(args.spans), rows, every)
...
def cmd_backfill(args):
    if args.image:
        ...
        report = backfill(blob, args, offset, args.log)
...
def cmd_service(args):
    ...
    if args.backfill:
        ...
        try:
            ...
            blob = read_history_tail(counter, args.backfill_bytes)
            if blob:
                say_backfill(backfill(blob, args, offset, log_path), offset)
    ...
    def row(averages):
        return log_row(time.time(), iv.cps(), iv.counts, iv.seconds,
                       averages, iv.peaks, SRC_LIVE) + "\n"
