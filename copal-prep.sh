...
# --- build identity ---------------------------------------------------------
# Three facts about where an image came from, written onto the card and
# readable from the running system. The point is to be able to hold a machine
# up against a build: "which press of make made this, and when".
#
# BUILD_ID is eight hex characters -- not a real GUID, and not pretending to
# be. It is a label for one press of make, and 4 billion of them is far more
# than a person will ever produce. Taken from the ENVIRONMENT when it is
# already set, which is what makes `make all` stamp its nine images with ONE
# id: the Makefile generates it once and exports it, so the images from a
# single press are recognisable as a set while still recording their own
# target. A direct ./copal-prep.sh run with nothing exported generates its own.
#
# It is random rather than a counter or a hash of the inputs. A counter needs
# somewhere to keep the count, and a content hash would give two presses the
# same id -- which is the opposite of what is wanted here, where the question
# is "is this the image I built an hour ago" and not "is this image
# reproducible".
build_id() {
    # /dev/urandom rather than $RANDOM: $RANDOM is 15 bits and seeded from the
    # pid on some shells, so two images built in the same second could collide.
    od -An -N4 -tx1 /dev/urandom 2>/dev/null | tr -d ' \n' | cut -c1-8
}
BUILD_ID="${BUILD_ID:-$(build_id)}"
case "$BUILD_ID" in
    [0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f]) ;;
    *) warn "BUILD_ID='$BUILD_ID' is not eight hex characters -- generating one"
       BUILD_ID=$(build_id) ;;
esac
...
# The Alpine payload's own date, as distinct from this build's. An image built
# today from a payload fetched in March is two different dates, and conflating
# them hides the more interesting one: "these packages are five months old" is
# a different problem from "this image is five months old".
#
# The mtime of the verified tarball is the honest answer available here. It is
# when this Mac obtained that payload, which for a cached file is when it was
# first downloaded -- not Alpine's publication date, which the tarball does not
# carry. Labelled 'fetched' rather than 'released' for exactly that reason.
PAYLOAD_DATE="(unknown)"
if [ -f "$CACHEDIR/$TARBALL" ]; then
    PAYLOAD_DATE=$(date -u -r "$CACHEDIR/$TARBALL" +%Y-%m-%dT%H:%M:%SZ 2>/dev/null \
                   || echo "(unknown)")
fi
...
# people and by scripts that have no business sourcing a shell file, it is
# useful with the card in a reader on a machine that will not boot, and it must
# not be lost if copal.conf is ever regenerated for another reason.
cat > "$MNT/copal-build" <<BUILDINFO
# Which press of make produced this image. Written by copal-prep.sh.
COPAL_BUILD_ID="${BUILD_ID}"
COPAL_BUILD_DATE="${BUILD_DATE}"
COPAL_BUILD_TARGET="${MODEL:-$ARCH}"
...
# One field out of the card's copal-build, which copal-prep.sh wrote. Read by
# grep rather than by sourcing: this runs in the middle of an install and the
# file is on a FAT partition anyone could have edited, and there is no reason
# for a value in it to become a variable in this shell.
build_field() {  # <name>
    [ -n "${BOOT:-}" ] || return 1
    [ -f "$BOOT/copal-build" ] || return 1
    sed -n "s/^$1=[\"']\{0,1\}//p" "$BOOT/copal-build" 2>/dev/null \
...
install_run_id() {
    ...
    if [ -s "$INSTALL_RUN_FILE" ]; then
        ...
        # The build id first, where there is one: an install is one image
        # installed once, so the run reads "which image" then "which time" --
        # 84d3398e-20260821-115732. The same image installed twice gives two
        # runs sharing a prefix, which is exactly the comparison worth making.
        _bid=$(build_field COPAL_BUILD_ID 2>/dev/null || true)
        INSTALL_RUN_ID="${_bid:+$_bid-}$(date +%Y%m%d-%H%M%S)"
    ...
    if [ ! -s "$INSTALL_TIMES" ]; then
        {
            ...
            # The image this record came from, as a comment rather than a
            # twelfth column: it is the same for every row in the file, and
            # repeating it 300 times would be a column that carries no
            # information. Readers skip '#' lines already, so this costs
            # nothing to anything that does not care.
            printf '#build\t%s\t%s\t%s\t%s\n' \
                "$(build_field COPAL_BUILD_ID || echo '?')" \
                "$(build_field COPAL_BUILD_DATE || echo '?')" \
                "$(build_field COPAL_BUILD_TARGET || echo '?')" \
                "$(build_field COPAL_ALPINE_VER || echo '?')"
...
install_claude_code() {
    ...
    # whole function unreachable: nothing in this script, in any stage, or in
    # the catalogue installs nodejs, so `command -v npm` was false on every
    # machine and Claude Code was never once installed by a run that asked for
    # it. Offering the dependency is the difference between a feature and a
    # dead branch.
...
Brave: use yt-brave, not --cookies-from-browser brave

    yt-brave URL
    yt-brave --profile 'Profile 1' URL
    yt-brave --list

    yt-dlp does know the word "brave", and on this machine it will still find
    nothing. Copal installs Brave as a Flatpak, and Flatpak redirects an
    application's config into its own tree, so the profile is not at
    ~/.config/BraveSoftware/Brave-Browser where yt-dlp looks. It is at

        ~/.var/app/com.brave.Browser/config/BraveSoftware/Brave-Browser

    yt-brave supplies that path and hands everything else to yt-dlp, so any
    yt-dlp option you would normally pass still works:

        yt-brave -f 'bv*+ba' --write-subs URL

    It checks the Flatpak location first and a native install second, so it
    keeps working if you install Brave some other way later.
...
# yt-dlp with Brave's cookies, which is two problems rather than one.
#
# THE FIRST is that yt-dlp already knows the word "brave" -- but it looks for
# the profile at ~/.config/BraveSoftware/Brave-Browser, and Copal installs
# Brave as a Flatpak. Flatpak redirects an application's config into its own
# per-app tree, so the profile is actually at
#
#     ~/.var/app/com.brave.Browser/config/BraveSoftware/Brave-Browser
#
# and the bare `--cookies-from-browser brave` finds nothing at all. The error
# it gives is about a missing database rather than about Flatpak, so it is not
# an obvious thing to work out from the message.
#
# THE SECOND is the keyring. Brave is Chromium underneath and encrypts cookies
# with a key from the desktop keyring. On a box with no keyring daemon running
# -- which i3 on Alpine is, unless you added one -- Chromium falls back to a
# fixed key, and yt-dlp calls that fallback "basictext". Auto-detection usually
...
install_ytbrave() {
    ...
    cat > /usr/local/bin/yt-brave <<'YTBRAVE'
#!/bin/sh
# SPDX-License-Identifier: MIT
# Generated by copal-init.sh.
#
#  yt-brave [--profile NAME] URL...   -- yt-dlp using Brave's cookies.
#
...
#   yt-brave --profile 'Profile 1' URL
#   yt-brave --list                 which profiles exist
#
# Anything else you pass goes straight through to yt-dlp:
#   yt-brave -f 'bv*+ba' --write-subs URL
set -eu

# Flatpak first, because that is how Copal installs it; then a native package
# for anyone who installed Brave some other way.
for _root in \
    "$HOME/.var/app/com.brave.Browser/config/BraveSoftware/Brave-Browser" \
    "$HOME/.config/BraveSoftware/Brave-Browser"
do
    [ -d "$_root" ] && { BRAVE_ROOT="$_root"; break; }
done

if [ -z "${BRAVE_ROOT:-}" ]; then
    echo "yt-brave: no Brave profile found. Looked in:" >&2
    echo "  ~/.var/app/com.brave.Browser/config/BraveSoftware/Brave-Browser  (Flatpak)" >&2
    echo "  ~/.config/BraveSoftware/Brave-Browser                            (native)" >&2
    echo "Start Brave once so it creates one, or use: yt-dlp --cookies cookies.txt" >&2
    exit 1
fi
...
case "${1:-}" in
    --list)
        echo "Profiles under $BRAVE_ROOT:"
        for _p in "$BRAVE_ROOT"/Default "$BRAVE_ROOT"/Profile\ *; do
            [ -d "$_p" ] && [ -f "$_p/Cookies" ] && echo "  $(basename "$_p")"
        done
        exit 0 ;;
    --profile) PROFILE="${2:?--profile needs a name}"; shift 2 ;;
esac

[ -d "$BRAVE_ROOT/$PROFILE" ] || {
    echo "yt-brave: no profile '$PROFILE'. Try: yt-brave --list" >&2; exit 1; }

# Brave holds the cookie database locked while it runs. yt-dlp copies it
# first, which usually works, but a browser mid-write is the one thing that
# turns this into an unexplainable failure -- so say it plainly rather than
# let it look like a cookie problem.
if pgrep -f 'com.brave.Browser|brave' >/dev/null 2>&1; then
    echo "yt-brave: Brave appears to be running. If this fails, close it and retry." >&2
fi

exec yt-dlp --cookies-from-browser "brave:$BRAVE_ROOT/$PROFILE" "$@"
YTBRAVE
    chmod 0755 /usr/local/bin/yt-brave
...
#
# THE COUNTER HAS TO BE VISIBLE BEFORE ANY OF IT MATTERS, and there are three
# ways it is not, with three different fixes. The one that costs an afternoon
# is the third:
#
#   1. nothing plugged in                    plug it in
#   2. the node is root:dialout              the user needs to be in dialout
#   3. THE KERNEL HAS NO USB-SERIAL DRIVER   change kernels
#
# Alpine's linux-virt -- which is what a UTM VM runs -- ships no ch341, no
# cp210x, no usbserial, no cdc-acm. Not "not loaded": not built. 884 modules
# and not one of them binds a USB serial adapter. So a counter passed through
# to a VM running linux-virt will never appear as /dev/ttyUSB0, dmesg says
# nothing because the device is never claimed, and every guide on the internet
...
# The '#build' comment line, if the file has one. Printed above the report so a
# set of numbers is never separated from the image that produced them --
# comparing two runs is the whole point, and that cannot be done if you cannot
# tell which build each came from.
buildline() {
    awk -F'\t' '/^#build/ {
        printf "  build %s   %s   %s   Alpine %s\n", $2, $3, $4, $5; exit
    }' "$TIMES" 2>/dev/null
}
